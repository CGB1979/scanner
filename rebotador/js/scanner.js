...
async function abrirScanner() {
    ...
    // Si el escáner ya está funcionando, no reiniciar la cámara.
...
function procesarCodigo(codigo) {
    ...
    const exactos = vehiculos.filter(
        v => key(v.chasis) === key(codigoNormalizado)
    );

    if (exactos.length) {
        mostrarVehiculo(exactos[0]);
    } else {
        mostrarNoEncontrado(codigoNormalizado);
    }
}
...
function mostrarNoEncontrado(codigo) {
    ...
    document.getElementById("scanInfo").innerHTML = `
        <div class="scan-info">
            <strong>Chasis escaneado:</strong>
            ${escapeHTML(codigo)}
            <br>
...
function borrarDesdeScan() {
    ...
    eliminarVehiculo(vehiculoActual.id);
...
/*
    Esta función se puede utilizar desde otros
    archivos después de cerrar el modal de ubicación.
...
function confirmarIngresoManual() {
    ...
    const resultados = vehiculos.filter(
        v => key(v.chasis).includes(key(codigo))
    );

    if (resultados.length === 1) {
        bloqueandoLectura = true;
        mostrarVehiculo(resultados[0]);
        return;
    }
